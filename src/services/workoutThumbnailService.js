...
function buildCompositeProfileThumbnail({ altitudes = [], powers = [] } = {}) {
  ...
  if (numericAltitudes.length < 2 && numericPowers.length < 2) {
    return null;
  }
  ...
  const powerMaxRaw = numericPowers.length ? Math.max(...numericPowers) : 1;
  ...
  const areaPath = numericAltitudes.length >= 2
    ...
    : "";
  const powerPath = numericPowers.length >= 2
    ? createLinePathFromSeries(numericPowers, {
        minValue: 0,
        maxValue: powerMax
      })
    : "";
