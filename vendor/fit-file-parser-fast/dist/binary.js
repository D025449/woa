...
function getBlobView(blob, startIndex, size) {
    return new DataView(blob.buffer, blob.byteOffset + startIndex, size);
}
function copyBlobRange(blob, startIndex, size) {
    const result = new Array(size);
    for (let i = 0; i < size; i++) {
        result[i] = blob[startIndex + i];
    }
    return result;
}
function readData(blob, fDef, startIndex, options) {
    if (fDef.type === 'uint8_array') {
        const array8 = new Array(fDef.size);
        for (let i = 0; i < fDef.size; i++) {
            array8[i] = blob[startIndex + i];
        }
        return array8;
    }
    if (fDef.endianAbility) {
        const dataView = getBlobView(blob, startIndex, fDef.size);
        try {
            switch (fDef.type) {
                case 'sint16':
                ...
                case 'uint32_array': {
                    const array32 = new Array(fDef.size >> 2);
                    let arrayIndex = 0;
                    for (let i = 0; i < fDef.size; i += 4) {
                        array32[arrayIndex++] = dataView.getUint32(i, fDef.littleEndian);
                    }
                    return array32;
                }
                case 'uint16_array': {
                    const array16 = new Array(fDef.size >> 1);
                    let arrayIndex = 0;
                    for (let i = 0; i < fDef.size; i += 2) {
                        array16[arrayIndex++] = dataView.getUint16(i, fDef.littleEndian);
                    }
                    return array16;
                }
            }
        ...
        const temp = copyBlobRange(blob, startIndex, fDef.size);
        return addEndian(fDef.littleEndian, temp);
    }
    if (fDef.type === 'string') {
        let endIndex = startIndex;
        const maxIndex = startIndex + fDef.size;
        while (endIndex < maxIndex && blob[endIndex]) {
            endIndex += 1;
        }
        return Buffer.from(blob.subarray(startIndex, endIndex)).toString('utf-8');
    }
    if (fDef.type === 'byte_array') {
        return copyBlobRange(blob, startIndex, fDef.size);
    }
    if (fDef.type === 'sint8') {
...
function readDataWithView(blob, dataView, viewOffset, fDef, startIndex, options) {
    if (fDef.type === 'uint8_array') {
        const array8 = new Array(fDef.size);
        for (let i = 0; i < fDef.size; i++) {
            array8[i] = blob[startIndex + i];
    ...
    if (fDef.endianAbility) {
        const offset = startIndex - viewOffset;
        try {
            switch (fDef.type) {
                case 'sint16':
                    return dataView.getInt16(offset, fDef.littleEndian);
                case 'uint16':
                case 'uint16z':
                    return dataView.getUint16(offset, fDef.littleEndian);
                case 'sint32':
                    return dataView.getInt32(offset, fDef.littleEndian);
                case 'uint32':
                case 'uint32z':
                    return dataView.getUint32(offset, fDef.littleEndian);
                case 'float32':
                    return dataView.getFloat32(offset, fDef.littleEndian);
                case 'float64':
                    return dataView.getFloat64(offset, fDef.littleEndian);
                case 'uint32_array': {
                    const array32 = new Array(fDef.size >> 2);
                    let arrayIndex = 0;
                    for (let i = 0; i < fDef.size; i += 4) {
                        array32[arrayIndex++] = dataView.getUint32(offset + i, fDef.littleEndian);
                    }
                    return array32;
                }
                case 'uint16_array': {
                    ...
                    let arrayIndex = 0;
                    for (let i = 0; i < fDef.size; i += 2) {
                        array16[arrayIndex++] = dataView.getUint16(offset + i, fDef.littleEndian);
                    }
                    return array16;
                }
            }
...
function getTypeMetadata(type) {
    ...
    if (!typeMap) {
        ...
        typeMetadataCache.set(type, metadata);
        return metadata;
    }
    let hasMask = false;
    const maskEntries = [];
        ...
        case 'uint8_array':
            if (Array.isArray(data)) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    result[i] = scale ? dataItem / scale + offset : dataItem;
                }
                return result;
            }
            return scale ? data / scale + offset : data;
        default:
            {
                ...
                const dataItem = {};
                for (let i = 0; i < metadata.maskEntries.length; i++) {
                    const [key, value] = metadata.maskEntries[i];
                    if (value === 'mask') {
...
export function readRecord(blob, messageTypes, developerFields, startIndex, options, startDate, pausedTime) {
    ...
    const fields = {};
    const messageName = messageType.messageName || getFitMessage(messageType.globalMessageNumber).name;
    const recordView = new DataView(blob.buffer, blob.byteOffset + startIndex + 1);
    const recordViewOffset = startIndex + 1;
    const rawFields = [];
    for (let i = 0; i < messageType.fieldDefs.length; i++) {
        const fDef = messageType.fieldDefs[i];
        const data = readDataWithView(blob, recordView, recordViewOffset, fDef, readDataFromIndex, options);
    ...
    for (const { fDef, data } of rawFields) {
        const field = fDef.isDeveloperField ? fDef.name : fDef.field;
    ...
    for (const { fDef, data } of rawFields) {
        if (fDef.isDeveloperField) {
            ...
            const { field, type, scale, offset } = fDef;
            if (field !== 'unknown' && field !== '' && field !== undefined) {
